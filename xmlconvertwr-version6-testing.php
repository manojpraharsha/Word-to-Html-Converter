...
//	li type 1 existance Conditions
    ...
    if($li_existance==null){ $li_existance_checkH3=1;}else{ $li_existance_checkH3=0; }
...
//	h2 existance Conditions	
    ...
    if($h3_existance_double_confirmation==24){
        $h3_existance_double_confirmation=1;
    }
    else{
        $h3_existance_double_confirmation=0;
    }
    if($h3_existance_double_confirmation_type2==24){
        $h3_existance_double_confirmation_type2=1;
    }
    else{
        $h3_existance_double_confirmation_type2=0;
    }
...
//	other existance Conditions		
	...
	$length2 = strlen($nodeValueExistance);    
	...
	if((!empty($nodeValueExistance))&&(($nodeValueExistance)!="")){
			if($no_of_childs>2){
				if (strpos($bredcrum_existance_doublecheck, 'PAGEREF') !== false) {		
				...
				else if((($x->evaluate("//wx:sect/w:p[".($i+1)."]/w:pPr/w:pStyle")->item(0)->attributes[0]->nodeValue)==2)&&($length2<100)){
					$original =$x->evaluate("//wx:sect/w:p[".($i+1)."]")->item(0)->nodeValue;
					$original = preg_replace('!\s+\s+!', '', $original);
                    $currentHeading = $original;
						echo "<h1>".$original."</h1>\n";
					}
					else if((($x->evaluate("//wx:sect/w:p[".($i+1)."]/w:pPr/w:pStyle")->item(0)->attributes[0]->nodeValue)==3)&&($length2<100)){
						$original =$x->evaluate("//wx:sect/w:p[".($i+1)."]")->item(0)->nodeValue;
						$original = preg_replace('!\s+\s+!', '', $original);
                        $currentHeading = $original;
						echo "<h1>".$original."</h1>\n";
					}
					else if((($x->evaluate("//wx:sect/w:p[".($i+1)."]/w:pPr/w:pStyle")->item(0)->attributes[0]->nodeValue)==4)&&($length2<100)){
						$original =$x->evaluate("//wx:sect/w:p[".($i+1)."]")->item(0)->nodeValue;
						$original = preg_replace('!\s+\s+!', '', $original);
                        $currentHeading = $original;
						echo "<h1>".$original."</h1>\n";
					}
					else if((($x->evaluate("//wx:sect/w:p[".($i+1)."]/w:pPr/w:pStyle")->item(0)->attributes[0]->nodeValue)==5)&&($length2<100)){
						$original =$x->evaluate("//wx:sect/w:p[".($i+1)."]")->item(0)->nodeValue;
						$original = preg_replace('!\s+\s+!', '', $original);
                        $currentHeading = $original;
						echo "<h1>".$original."</h1>\n";
					}
				else if((($x->evaluate("//wx:sect/w:p[".($i+1)."]/w:pPr/w:pStyle")->item(0)->attributes[0]->nodeValue)==6)&&($length2<100)){
						$original =$x->evaluate("//wx:sect/w:p[".($i+1)."]")->item(0)->nodeValue;
						$original = preg_replace('!\s+\s+!', '', $original);
                        $currentHeading = $original;
						echo "<h1>".$original."</h1>\n";
					}
				else if((($x->evaluate("//wx:sect/w:p[".($i+1)."]/w:pPr/w:pStyle")->item(0)->attributes[0]->nodeValue)==7)&&($length2<100)){
						$original =$x->evaluate("//wx:sect/w:p[".($i+1)."]")->item(0)->nodeValue;
						$original = preg_replace('!\s+\s+!', '', $original);
                        $currentHeading = $original;
						echo "<h1>".$original."</h1>\n";
                    ...
                    else if((($h3_existance==1)||($h3_existance_thriple_confirmation==1)||($h3_existance_confirmation==1)||($h3_existance_double_confirmation)||($h3_existance_double_confirmation_type2)||($h3_more_existance==1)||($h3_existance_more_confirmation==1))&&(($li_existance_checkH3)&&($length2<100))){
					...
					else if((($h3_existance==1)||($h3_existance_thriple_confirmation==1)||($h3_existance_confirmation==1)||($h3_existance_double_confirmation)||($h3_existance_double_confirmation_type2)||($h3_more_existance==1)||($h3_existance_more_confirmation==1))&&(($li_existance_checkH3)&&($length2<100)&&($h3_existance_double_confirmation_type3<2))){
						$original =$x->evaluate("//wx:sect/w:p")->item($i)->nodeValue;						
						$original = preg_replace('!\s+\s+!', '', $original);
						echo "<h2>".$original."</h2>\n";
					}
